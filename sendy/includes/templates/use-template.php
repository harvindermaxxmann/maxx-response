...
<?php 
    ...
    $original = decrypt_int($_SESSION['listId']);
    if ($original > 0) {
        $list_id = $original;
    } else {
	...
	if(!$ares_id) //Create new campaign with template
		$q = 'INSERT INTO campaigns (userID, app, from_name, from_email, reply_to, query_string, title, html_text, plain_text, to_send_lists, wysiwyg, opens_tracking, links_tracking) VALUES ('.get_app_info('main_userID').', '.$aid.', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.$query_string.'", "'.$template_name.'", "'.addslashes($html_text).'", "'.addslashes($plain_text).'", "'.$list_id.'", 1, '.$opens_tracking.', '.$links_tracking.')';
	else
