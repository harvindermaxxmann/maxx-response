...
unset($_SESSION['campaign_subject_name']);
...
$_SESSION['campaign_subject_name'] = $_GET['campaign_subject_name'];
$_SESSION['sr_name'] = $_GET['sr_name'];
$_SESSION['subject_name'] = $_GET['subject_name'];
$_SESSION['status'] = $_GET['status'];	
$_SESSION['listId'] = $_GET['listId'];	
$_SESSION['redirect_from'] = $_GET['redirect_from'];	
...
$pass_data = $_GET['pass'];
	...
	if(isset($email_data)){
		$email = mysqli_real_escape_string($mysqli, $email_data);
		$pass = mysqli_real_escape_string($mysqli, $pass_data);
	}else{
...
else
{
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
		...
		{	// if we come from crm		
			if(isset($_SESSION['redirect_from']) && !empty($_SESSION['redirect_from']) && $_SESSION['redirect_from'] == 'campaigns' ){
				$c_id = $_SESSION['c_id'];
				$c_id = urlencode($c_id);
				// Redirect to the Sendy edit URL
				header("Location: https://maxxresponse.com/sendy/edit?i=1&c={$c_id}");
			}else{
				header("location: https://maxxresponse.com/sendy/create?i=1");
			}
