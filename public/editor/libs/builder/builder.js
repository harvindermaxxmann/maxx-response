...
/* drag and drop */
	...
	saveAjax: function(actiontype,fileName, startTemplateUrl, callback)
	{
		//$('#Loader').show();
		var ref = $('#ref').text();
		var type = $('#draftype').text();
		var data = {};
		...
		$.ajax({
			type: "POST",
			url: '/save-template',//set your server side save script url
			data: data,
			cache: false,
			success: function (data) {
				if(data.status =="success"){
					if(data.type=="newsletter"){
						$('#Loader').hide();
						window.location.href = "/newsletter-summary?ref="+ref;
					}else if(data.type=="landing-page"){
						if(data.redirect !="save-next"){
							window.location.href = "/html/editor?type=landing-page&a="+data.redirect+"&ref="+data.ref;
						}else{
							$('#Loader').hide();
							window.location.href = "/lp-settings?ref="+data.ref;
						}
					}
				}else{
					alert('Some Went wrong! Please try again or hard refresh the page');
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(actiontype,url, null, function (data) {
