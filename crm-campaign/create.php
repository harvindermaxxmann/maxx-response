...
if(isset($admin_email)) { 
		$q = "SELECT id FROM `template` ORDER BY id DESC LIMIT 1";
			$r = mysqli_query($mysqli, $q); // Changed $r to mysqli_query($mysqli, $q)
...
$(document).ready(function() {
    window.location.href = "https://maxxmanncommunications.com/crm-campaign/includes/templates/use-template.php?i=3&t=<?php echo $temp_id; ?>";
});
</script>
<?php } ?>
<?php
			    	...
			    	<div class="control-group">
				    	<div class="controls">
					      <?php 
							  ...
							  $admin__email = $_SESSION['admin_email'];
							  if(!empty($admin__email)){
								$admin__email = $admin__email;
							  }else{
								$admin__email = get_app_data('from_email');
							  } 
					      ?>
			              <input type="text" class="input-xlarge" <?php if(get_app_info('is_sub_user') && verify_identity($main_email)!='verified' && get_app_info('s3_key')!='' && get_app_info('s3_secret')!='') echo 'readonly="readonly"';?> id="from_email" name="from_email" placeholder="<?php echo _('name@domain.com');?>" value="<?php echo $admin__email;?>">
			            </div>
			        </div>
